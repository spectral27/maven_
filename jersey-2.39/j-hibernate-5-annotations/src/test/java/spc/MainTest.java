...
public class MainTest {
    ...
    public static void beforeAll() {
        try {
            ...
            DriverManager.getConnection(
                    "jdbc:h2:mem:database01;INIT=runscript from 'classpath:test_main.sql'",
                    "username",
                    "password"
            );
    ...
    public void mainTest() {
        ...
        client.target("http://localhost:8080/objects")
                .request()
                .post(Entity.entity(junit, MediaType.APPLICATION_JSON))
                .close();

        List<JavaObject> javaObjects = client.target("http://localhost:8080/objects")
                .request()
                .get(new GenericType<>() {});
